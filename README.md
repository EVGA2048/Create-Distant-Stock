...
安山岩承重，黄铜做机构，霁青以太只出现在接口、导轨和指示上。远仓打包机沿用 Create 原版结构，换成淡蓝色外观并产出远仓包裹；港接在打包机、皮带和漏斗后面。
...
| 远仓打包机 | `remote_packager` | Create 打包机的淡蓝色变体，产出远仓包裹 |
| 远仓包裹 | `remote_package` | 保留地址与内容数据的淡蓝色包裹 |
...
- 不是无人机系统；远仓打包机沿用 Create 的打包逻辑，但会产出淡蓝色远仓包裹。
