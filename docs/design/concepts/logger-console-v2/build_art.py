...
def model(state='alarm', lamps=3):
    ...
    return {'parent':'minecraft:block/block','loader':'neoforge:composite',
            'credit':'Distant Stock / logger console V2 / Create native glass RGBA and brown sockets; custom miniature layout',
...
def mesh(data):
    ...
    for part in data.get('children',{'all':data}).values():
        ...
        for f in fs:
            if 'digit_' in f['material'] and f['material'].endswith('_lit'): f['emissive']=True
...
def main():
    ...
    label(sheet,(48,85),'DISTANT STOCK   /   CREATE NIXIE GLASS REVISION   /   02',17,MUTED)
    d.line((46,121,1554,121),fill='#b1b6a7')
    sheet.alpha_composite(draw(models['alarm'],(770,620),32,28,17),(20,140))
    label(sheet,(60,748),'原版棕橙半透明玻璃 · 管内悬浮数字 · 深棕底座',21)
    label(sheet,(60,784),'16×16 面板 / 原版玻璃 alpha 96 / 管体前凸 4。',18,MUTED)
