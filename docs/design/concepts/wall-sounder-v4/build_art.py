...
def assets():
    ...
    # Add one centre column and retain a middle depth row: 6x5x3 glass.
    ...
    islands={
        ...
        'side':Image.fromarray(native_side[:,[0,2,4]]),
        'top':Image.fromarray(native_top[np.ix_([0,2,4],cols)]),
        # Closed glass underside now that the socket below it has been removed.
        'bottom':Image.fromarray(native_top[np.ix_([4,2,0],cols)]),
    }
...
def model(color='red',state='off',wall=False):
    ...
    for y in (9,11,13):
        ...
        e.append(rail)
    # Intermediate proportions, still without a supporting collar or iron foot.
    lamp=box('glass_bulb',[5,3,11],[11,8,14],f'{color}_{state}_front')
    ...
    lamp['faces']['south']['uv']=[6,0,0,5]
    lamp['faces']['west']['uv']=[3,0,0,5]
    for side in ('up','down'):lamp['faces'][side]['rotation']=180
    filament=box('filament',[6.5,3.5,12.5],[9.5,7.5,12.5],'filament_'+state)
    # Original 3x4 U-shaped filament, centred in the three-unit glass depth.
...
def main():
    ...
    label(sheet,(40,26),'壁挂声光报警器 V4 / 折中比例',35)
    label(sheet,(42,81),'6×5×3 灯罩 · 宽度收回一格、厚度增加一格 · 无底座、安山主题',20,'#717a6a')
    sheet.alpha_composite(draw(models['red','off'],(700,650),43),(5,125))
    label(sheet,(70,778),'8×12 安山机身 / 下方没有悬伸底座',24)
    label(sheet,(70,820),'灯罩直接贴背板，前端与栅条齐平。',20,'#717a6a')
    ...
    label(sheet,(45,908),'灯罩由 V3 的 7×5×2 调整为 6×5×3；相对背板前凸 3。',23)
