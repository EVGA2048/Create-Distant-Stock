...
def model(active=None, brass_rings=True):
    ...
    for y in (5, 11, 17, 23):
        ring = 'brass_ring' if brass_rings and y in (11, 17) else 'ring'
...
def main():
    ...
    sheet.convert('RGB').save(HERE / 'factory-stack-light-v3.png')
    draw_model(model(), (640, 900), 30).convert('RGB').save(HERE / 'three-quarter.png')
    comparison = Image.new('RGBA', (1100, 1000), BG)
    txt(comparison, 38, 24, '工厂三色灯 / 黄铜连接环对比', 30)
    txt(comparison, 38, 72, '仅替换灯泡之间的两道连接环；几何、像素密度与灯罩保持一致', 18, '#6b7870')
    for x, brass_rings in [(15, False), (565, True)]:
        comparison.alpha_composite(draw_model(model(brass_rings=brass_rings), (520, 770), 27), (x, 122))
        txt(comparison, x+102, 906, 'V3 / 黄铜连接环' if brass_rings else 'V2 / 安山连接环', 25)
    txt(comparison, 38, 966, 'Create 原版材质 · 1 纹素 / 模型单位 · 单层灯罩，无灯芯 · 项目 Python 渲染器', 16, '#6b7870')
    comparison.convert('RGB').save(HERE / 'brass-comparison.png')
    print(f'UV density verified: {verify(model())} faces, 1 texel / model unit; no inner core.')
    print(HERE / 'brass-comparison.png')
