...
def base():
    ...
    # Preserve Create's chassis shading and silhouette, in a blue metal palette.
    shell={
        (153,90,61):'#507b9b', (118,70,45):'#35536e',
        (255,249,199):'#d9ebee', (96,47,36):'#293f53',
        (255,254,245):'#f3f6e9', (181,120,73):'#699dbc',
        (70,71,72):'#3e596b', (50,52,52):'#293d4b',
        (251,204,104):'#a0c9df', (255,235,148):'#c2dfeb',
        (90,90,90):'#58798b', (211,161,85):'#80afcb',
        (99,104,107):'#678c9f', (208,208,208):'#d9ebee',
        (169,169,169):'#adcbd8', (136,136,136):'#87acbf',
    }
    original=im.copy()
    for y in range(16):
        for x in range(16):
            ...
            if p[3]:
                rgb=tuple(bytes.fromhex(shell[p[:3]][1:]))
                im.putpixel((x,y),rgb+(p[3],))
    # Darken the inset plate relative to the ice-blue outer chassis.
    old={(153,90,61,255):'#405f75',(118,70,45,255):'#2a4257',(181,120,73,255):'#587d94'}
    d=ImageDraw.Draw(im)
    for y in range(5,11):
        for x in range(4,12):
            if original.getpixel((x,y)) in old: d.point((x,y),fill=old[original.getpixel((x,y))])
    return im
...
def main():
    ...
    label(sheet,(42,27),'以太构件 V2 / 蓝色外壳',36)
    label(sheet,(44,82),'冰蓝金属外圈 · 蓝灰侧件 · 以太晶核与少量铜质机构 · 16×16',20,'#717a6a')
