...
def assets():
    ...
    # Broaden the native five-texel glass by adding two centre columns, then
    # keep only front/back edge rows on the shallow top/bottom. No resampling.
    native_side=np.asarray(detail.crop((27,5,32,10)))
    native_top=np.asarray(detail.crop((27,0,32,5)))
    cols=[0,1,2,2,2,3,4]
    islands={
        'front':Image.fromarray(native_side[:,cols]),
        'side':Image.fromarray(native_side[:,[0,4]]),
        'top':Image.fromarray(native_top[np.ix_([0,4],cols)]),
        # Closed glass underside now that the socket below it has been removed.
        'bottom':Image.fromarray(native_top[np.ix_([4,0],cols)]),
    }
    for color,hue in [('red',.008),('orange',.075)]:
        for state in ('off','on'):
            for face,source in islands.items():
                im=source.copy()
...
def model(color='red',state='off',wall=False):
    ...
    for y in (9,11,13):
        ...
        e.append(rail)
    # Wide, shallow wall-mounted lens. No supporting collar or iron foot.
    lamp=box('glass_bulb',[4.5,3,12],[11.5,8,14],f'{color}_{state}_front')
    for side in ('east','west'):lamp['faces'][side]['texture']=f'sounder_study:{color}_{state}_side'
    lamp['faces']['up']['texture']=f'sounder_study:{color}_{state}_top'
    lamp['faces']['down']['texture']=f'sounder_study:{color}_{state}_bottom'
    lamp['faces']['south']['uv']=[7,0,0,5]
    lamp['faces']['west']['uv']=[2,0,0,5]
    for side in ('up','down'):lamp['faces'][side]['rotation']=180
    filament=box('filament',[6.5,3.5,13],[9.5,7.5,13],'filament_'+state)
    # Original 3x4 U-shaped filament, one unit behind the front glass.
...
def main():
    ...
    draw(models['red','off'],yaw=90,pitch=0).save(HERE/'sounder_side_profile.png')
    draw(model('red','off',True),scale=31,center=(8,8,14)).save(HERE/'sounder_on_wall.png')
    sheet=Image.new('RGBA',(1460,1130),BG)
    label(sheet,(40,26),'壁挂声光报警器 V3 / 宽扁灯罩',35)
    label(sheet,(42,81),'移除下托座与铁脚 · 灯罩加宽、贴近背板 · 保留安山主题与半透明玻璃',20,'#717a6a')
    sheet.alpha_composite(draw(models['red','off'],(700,650),43),(5,125))
    label(sheet,(70,778),'8×12 安山机身 / 下方没有悬伸底座',24)
    label(sheet,(70,820),'宽灯罩直接贴背板，前面与蜂鸣器外壳齐平。',20,'#717a6a')
    ...
    label(sheet,(45,908),'灯罩由 5×5×5 改为 7×5×2；相对背板前凸由 5.5 减为 2。',23)
