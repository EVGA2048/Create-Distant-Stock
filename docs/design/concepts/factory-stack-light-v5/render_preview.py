...
atlas_patch('base_side', frame_patch(andesite, 6).crop((0, 2, 6, 5)))
...
def model(active=None):
    ...
    for original_y in (5, 11, 17, 23):
        if original_y == 5:
            housing = element('extended_lower_base', [5, 5, 5], [11, 8, 11],
                              'study:base_side', 'study:ring_top', 'study:ring_top')
...
def main():
    ...
    txt(sheet, 38, 70, 'V5 · 黄铜顶盖与连接环 · 下灯座仅加高、不加宽 · Python 模型预览', 18, '#6b7870')
    ImageDraw.Draw(sheet).line((36, 108, 1464, 108), fill='#bbc6bd')
    sheet.alpha_composite(draw_model(model(), (580, 770), 28), (18, 130))
    txt(sheet, 64, 932, '黄铜顶盖、连接环 / 安山下灯座', 23)
    txt(sheet, 64, 970, '下灯座 6 × 6 × 3 · 总高 26 · 灯罩 5 × 5 × 5', 17, '#6b7870')
    ...
    sheet.convert('RGB').save(HERE / 'factory-stack-light-v5.png')
    draw_model(model(), (640, 900), 30).convert('RGB').save(HERE / 'three-quarter.png')
    views = Image.new('RGBA', (1000, 960), BG)
    txt(views, 38, 24, '工厂三色灯 V5 / 黄铜顶盖 + 加高下灯座', 28)
    txt(views, 38, 70, '下灯座保持原来 6 × 6 宽度，只加高至 3 像素；无额外开孔', 18, '#6b7870')
    views.alpha_composite(draw_model(model(), (570, 780), 27), (0, 115))
    views.alpha_composite(draw_model(model(), (400, 780), 27, 0, 0), (570, 115))
    txt(views, 220, 910, '三分之四视图', 20)
    txt(views, 730, 910, '正视图', 20)
    views.convert('RGB').save(HERE / 'preview-v5.png')
    print(f'UV density verified: {verify(model())} faces, 1 texel / model unit; no inner core.')
    print(HERE / 'preview-v5.png')
