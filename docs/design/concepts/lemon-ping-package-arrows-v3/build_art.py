...
for y0 in (1,13):
 for ox,oy,rows,palette in (
  ...
  (80,y0*2+15,[r[::-1] for r in arrow[::-1]],('#fff3d3','#e5dbb9','#b2aa8b'))):
...
(OUT/'README.md').write_text('柠檬黄诊断包往返箭头方案：两个错开的配对短箭头，上方深灰向右、下方暖白向左，箭头轮廓为对称阶梯像素；深灰采用灰绿三色，暖白采用奶油白/麦灰三色，上缘亮、下缘暗，并少量透出纸箱纹理和褪色像素。保持无背景，不增加边框或端点。只印在东西两个无胶带侧面，保持现有箱体颜色、模型与 UV。使用半模型单位的硬边像素网格，原有箱体纹素不变。此前图标另存，未修改正式游戏资源。\n')
